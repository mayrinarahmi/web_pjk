...
class Edit extends Component
{
    ...
    {
        $targetBulan = TargetBulan::findOrFail($id);
        $this->targetBulanId = $targetBulan->id;
        $this->tahunAnggaranId = $targetBulan->tahun_anggaran_id;
        $this->nama_kelompok = $targetBulan->nama_kelompok;
        $this->bulan = json_decode($targetBulan->bulan);
        $this->persentase = $targetBulan->persentase;
    ...
    public function update()
    {
        ...
        session()->flash('message', 'Target bulan berhasil diperbarui.');
        return redirect()->route('target-bulan.index');
    }
    
    public function render()
    {
        return view('livewire.target-bulan.edit');
    }
}
